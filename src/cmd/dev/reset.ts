...
export const resetCmd: Command = {
    ...
    async execute(api: CommandAPI) {
        ...
        if (!userId) {
            ...
            warningText = table === 'all'
                ? `Właśnie próbujesz usunąć **WSZYSTKIE** dane użytkownika **${displayName}**.`
                : `Właśnie próbujesz usunąć dane z tabeli **${table}** dla użytkownika **${displayName}**.`;
        }
        ...
        collector.on('collect', async (interaction) => {
            if (interaction.customId == 'reset-confirm') {
                try {
                    switch (table) {
                    case 'economy':    await db.reset.economy(userId); break;
                    case 'leveling':   await db.reset.leveling(userId); break;
                    case 'cooldowns':  await db.reset.cooldowns(userId); break;
                    case 'warns':      await db.reset.warns(userId); break;
                    case 'reputation': await db.reset.reputation(userId); break;
                    case 'all':        await db.reset.all(userId); break;
                    }
