...
const WorkMessages: MessageCallback[] = [
    ...
    amount => `Napisałeś skrypt który robił twoją pracę za ciebie i zarobiłeś **${formatMoney(amount)}**. Automatyzacja to przyszłość.`,
    amount => `Naprawiłeś komputer który miał tylko odłączony kabel zasilania i zarobiłeś **${formatMoney(amount)}**.`,
    amount => `Sprzedałeś komuś "gamingową" myszkę z RGB i zarobiłeś **${formatMoney(amount)}**. FPS dalej ten sam.`,
    amount => `Znalazłeś stare bitcoiny na dysku i sprzedałeś część za **${formatMoney(amount)}**. Gdybyś tylko nie sprzedał w 2013...`,
    amount => `Dostałeś **${formatMoney(amount)}** za ustawienie komuś trybu ciemnego w aplikacji.`,
    amount => `Ktoś zapłacił ci **${formatMoney(amount)}** za zrobienie strony w WordPressie. Skopiowałeś template w 5 minut.`,
    amount => `Sprzedałeś kurs "Jak zarabiać w internecie" i zarobiłeś **${formatMoney(amount)}**. Ironia losu.`,
    amount => `Przekonałeś kogoś że kabel HDMI za 300zł daje lepszą jakość i zarobiłeś **${formatMoney(amount)}**.`,
    amount => `Znalazłeś **${formatMoney(amount)}** w starym portfelu. Inflacja i tak już zjadła połowę.`,
    amount => `Napisałeś 200 linijek kodu żeby uniknąć 5 minut pracy i zarobiłeś **${formatMoney(amount)}**.`,
