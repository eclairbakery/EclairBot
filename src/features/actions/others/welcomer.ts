...
export const welcomeNewUserAction: Action<UserEventCtx> = {
    ...
    constraints: [
        () => cfg.legacy.features.welcomer.enabled ? Ok : Skip,
    ],
    callbacks: [
        async (member) => {
            if (await watchNewMember(member) == 'kicked') return MagicSkipAllActions;

            for (const role of cfg.legacy.features.welcomer.freeRolesForEveryone) {
                try {
            ...
            const welcomeChannel = await client.channels.fetch(cfg.legacy.features.welcomer.channelId);
            if (welcomeChannel == null || !welcomeChannel.isSendable()) return;

            const generalChannel = await client.channels.fetch(cfg.legacy.features.welcomer.general);
            ...
            if (member.user.id == StartItId) {
                ...
                await welcomeChannel.send('<:emoji1:1410551894023082027>' + (cfg.legacy.features.welcomer.welcomeMsgs[Math.floor(Math.random() * cfg.legacy.features.welcomer.welcomeMsgs.length)]).replace('<mention>', cfg.legacy.features.welcomer.mentionNewPeopleInLobby ? `<@${member.user.id}>` : member.user.username));
...
export const sayGoodbyeAction: Action<UserEventCtx> = { 
    ...
    callbacks: [
        async (member) => {
            const channel = await client.channels.fetch(cfg.legacy.features.welcomer.channelId);
            ...
            await channel.send('<:emoji2:1410551857935290368>' + (cfg.legacy.features.welcomer.goodbyeMsgs[Math.floor(Math.random() * cfg.legacy.features.welcomer.goodbyeMsgs.length)]).replace('<mention>', cfg.legacy.features.welcomer.mentionNewPeopleInLobby ? `<@${member.user.id}>` : member.user.username));
        }
    ],
};
