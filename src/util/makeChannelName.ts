...
export function makeChannelName(data: { name: string; emoji: string; }) {
    ...
    return `${cfg.channelsConfiguration.emojiPlacement == 'before-name' ? makeEmojiForChannelName(data.emoji) : '' }${data.name.replace(' ', cfg.channelsConfiguration.spaceReplacement ?? '-')}${cfg.channelsConfiguration.emojiPlacement == 'after-name' ? makeEmojiForChannelName(data.emoji) : '' }`;
}