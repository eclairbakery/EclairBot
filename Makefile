...
TMP_DIR ?= /tmp/eclairbot
PKG_DIR ?= $(HOME)/.cache/deno/npm
CONFIG_FILE ?= bot/config.js
DATABASES ?= bot.db,bot.db-journal

DENO_IO_PERMS_FLAGS   = --allow-read=$(CONFIG_FILE),.env,$(DATABASES),$(CACHE_DIR),$(PKG_DIR),$(TMP_DIR),. \
						--allow-write=$(CONFIG_FILE),$(DATABASES),$(CACHE_DIR),$(PKG_DIR),$(TMP_DIR),.
DENO_PERMISSION_FLAGS = $(DENO_IO_PERMS_FLAGS) --allow-net --allow-sys --allow-ffi --allow-env --allow-run=yt-dlp
